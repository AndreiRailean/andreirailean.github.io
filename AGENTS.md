...
The role that **builds a piece** — from a statement of Andrei's imagination, or a change to one already here — and puts it in front of him. It partitions the far side of the steward's question rather than moving its line: a visitor sees no difference, the steward's; a visitor would see it and something has to be built or isolated before it can be judged, **the experiment writer's**; a visitor would see it and the question is which is better, Andrei's. It builds and never decides. **Its bias is always toward something that can be interacted with**: all of Andrei's feedback follows interaction, so the first interactable build is a gate rather than a step, and no spec or plan document is written for a piece. A standing role, invoked with `/experiment-writer`. See `docs/agents/experiment-writer.md`, `docs/adr/20260919-the-experiment-writer-is-the-third-gate.md` for why the gate exists and why one agent per experiment was rejected instead, and `docs/adr/20260919-an-interactable-build-precedes-the-design-conversation.md` for why building outranks designing here.
