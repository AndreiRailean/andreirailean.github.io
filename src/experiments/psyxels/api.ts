 ...
 * not.
 *
 * `run()` is this piece's counterpart of Dangler's `settle()`. Repacking is the
 * slow motion here — a square reconsiders its size a few times a minute — so a
 * still of frame one is a still of a field that has never changed size, which is
...
export type ExperimentApi = {
  ...
  /** Draw the squares the packing chose. */
  ...
  awake: () => boolean
  /** What the field is made of, how well it matches the subject, and what it costs. */
  stats: () => PsyxelsStats
}
...
export function announceApi(): void {
  const lines = [
    ...
    ["experiment.stats()", "pixels, sizes, how well the subject survives, fps"],
  ...
  console.log(`%cPsyxels%c is scriptable from here.\n\n${body}\n`, "font-weight:600", "font-weight:400")
}

export function createApi(controls: Controls<Settings>, wakeLock: WakeLock, scene: Psyxels): ExperimentApi {
  return {
