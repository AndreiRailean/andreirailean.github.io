...
screen pixels and a psyxel is anywhere between three and two hundred of them
across. So: a **psyxel**. It owns a square of the picture, it shows one frame of
...
**Same size everywhere** goes first. A psyxel can be three screen pixels
...
**One colour each** goes next. A psyxel is not a colour, it is a small animation:
a frame from a finite vocabulary of signs, held for a while, then swapped for
another. Which one comes next is the psyxel's own business.
...
no cheap answer to "this psyxel should be smaller now" except repacking
everything around it.

So the field is a **quadtree**. A square is either one psyxel or four, and each
of those four is either one psyxel or four, recursively. The cover is exact by
...
it. That correction is not cosmetic. **A psyxel is ended by the first of its
...
frames is how many features differ. A psyxel picks its next frame with a strong
preference for a near neighbour, so it grows a stroke, then acquires a ring, then
loses the stroke. That is what makes a psyxel read as one thing changing its mind
...
itself round like a pen stroke, starting wherever that psyxel happens to start,
...
Nothing in the field runs on a shared clock. Each psyxel has its own rate, its own
...
Where a psyxel's mark sits is its own business too. The packing is a
subdivision, so the squares are a lattice, and a coarse psyxel can only ever
...
how far one psyxel may wander from another, from a tinted monochrome to the whole
wheel present at once in one letter.

Between them they cover the range the piece was after: the same A, sober and
white, or hallucinating, without a psyxel moving.
...
A psyxel near the boundary is there _by its own luck_: how far its coverage sits
...
that declines is one psyxel where there would have been four — so it is kept to a
...
The frontier of the piece is how far a psyxel may sit outside the subject before
...
The coarsest psyxel is a share of the frame's shorter side rather than a number
...
is answered in four lookups and a two-hundred-pixel psyxel costs no more than a
three-pixel one. A seeded generator salted with each square's own depth and
position, so a psyxel's life is a pure function of where it is — raising the
