...
export type NumericKey = Exclude<keyof Settings, "flow" | "palette" | "dusk">
...
export const CONTROLS: Control[] = [
  ...
  },
  {
    ...
    step: 0.05,
    format: (v) => `${Math.round(v * 100)}%`,
    hint: "Fraction who have somewhere in frame to be rather than somewhere past it. They walk to a spot, stand or sit for a while — a group sitting arranges itself in a rough ring so it can talk — and eventually get up and leave. At 0 the frame is all traffic.",
  },
  {
    kind: "range",
  ...
  },
  {
    ...
    step: 0.02,
    format: (v) => v.toFixed(2),
    hint: "How much colour the ground has. Near 0 it is a neutral grey the crowd sits on top of; high up it is a stated green or clay and the pastels have to work against it.",
  },
  {
    kind: "slider",
    key: "spread",
...
function namesASetting(params: URLSearchParams): boolean {
  ...
  if (params.get("dusk") !== null) return true
