...
export type Settings = {
  ...
  dissolve: number
  /** How much faster a large bubble's film drains than a small one's. */
  fragile: number
...
export const CONTROLS: Control[] = [
  ...
  },
  {
    ...
    hint: "How big a bubble is when it reaches the surface, from the smallest to the largest. Every bubble you see bigger than this got that way by swallowing others, which is the only way anything grows here — so bring the top of the band down and the big ones have to be earned rather than delivered. Gas is a quantity rather than a count, so narrowing this band does not thin the foam out; it gives you more, smaller bubbles covering the same water.",
  },
  {
    kind: "slider",
    key: "churn",
  ...
  },
  {
    ...
    step: 0.02,
    format: (v) => v.toFixed(2),
    hint: "How much faster a large bubble's film drains than a small one's — which is to say, how much shorter a big bubble's life is. A wide film held up against gravity thins quicker than a small one, so at any value above 0 growth has a cost and the foam settles at a size where coalescence and drainage balance, rather than running away. At 0 every bubble drains at the same rate whatever its size, and a big one can only die by bursting.",
  },
  {
    kind: "slider",
    key: "popSize",
...
export const PRESETS: { label: string; hint: string; settings: Settings }[] = [
  {
    label: "deep water",
    hint: "Jets two metres down, so the gas arrives gently and the swirl does the rest. About eleven seconds for a bubble to cross the frame. The starting point.",
    settings: {
      jets: 3,
      layout: "ring",
      spin: "alternate",
  ...
  {
    label: "shallow",
    hint: "The same tub with the jets just under the surface. Depth is the only setting that differs, so what it does can be moved rather than described: the boil goes from a third of a metre wide to seven centimetres, and the surface stops being calm.",
    settings: {
      jets: 3,
      layout: "ring",
      spin: "alternate",
  ...
  {
    label: "rolling boil",
    hint: "Shallow water and jets that are frankly industrial. This is what the surface does when the gas has no depth to calm down in.",
    settings: {
      jets: 6,
      layout: "ring",
      spin: "alternate",
  ...
  {
    label: "slick",
    hint: "Little gas and almost certain coalescing, so a few big circles drift and burst. Fragile is well up, so the big ones do not outstay their welcome.",
    settings: {
      jets: 3,
      layout: "scatter",
      spin: "loose",
  ...
  {
    label: "meniscus",
    hint: "Drawn as hairline rings rather than discs, with few enough large bubbles for a ring to read. Closer to what foam on real water looks like from above.",
    settings: {
      jets: 3,
      layout: "ring",
      spin: "alternate",
