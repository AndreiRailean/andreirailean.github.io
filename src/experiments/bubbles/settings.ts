...
export type Settings = {
  ...
  cling: number
  /** Film failures per second between two touching bubbles. */
  merge: number
  /** Share of those failures that destroy a bubble instead of joining two. */
  rupture: number
  ...
  count: number
  /** Wall thickness of a ring, in screen pixels. Unused by `disc`. */
  rim: number
...
export const CONTROLS: Control[] = [
  ...
  },
  {
    ...
    hint: "The slow flow back toward the middle that a closed tub must have — everything the jets push out has to come from somewhere. **Where it cancels the jets' push there is a standing ring**: foam carried outward arrives and cannot go further, foam beyond it is brought back. Turning this up pulls the ring in and turning it down pushes it out, and past a point the ring is outside the picture and you will never know it is there — `experiment.stats().ring` gives its radius in millimetres so you can widen `frame` to meet it. With churn and waver at zero it comes out as a drawn circle; with either of them up it breathes.",
  },
  {
    kind: "slider",
    key: "wave",
  ...
  },
  {
    ...
    step: 0.01,
    scale: "log",
    format: (v) => (v >= 1 ? `${v.toFixed(1)}/s` : `1 per ${Math.round(1 / v)}s`),
    hint: "How often the film between two touching bubbles gives way, per second of contact. **This is the control that decides whether you get foam.** Near the top a pair joins the instant it touches, so bubbles never accumulate — there is nothing to see but a size that keeps climbing. Down at one event every few seconds, touching bubbles stay separate neighbours for a long time and pack into a raft, which is what foam is. A merge keeps the area of both, so radius grows as the square root and it takes four bubbles to double one.",
  },
  {
    kind: "slider",
    key: "rupture",
    ...
    hint: "Of the film failures above, the share that destroys a bubble instead of joining two. A film between neighbours can drain through and let them merge, or it can take the outer wall with it and the bubble is simply gone — and the bigger of the pair is likelier to be the one that goes, because it is holding up more film. At 0 every contact that resolves is a merge and the foam can only coarsen.",
  },
  {
    kind: "slider",
    key: "bounce",
  ...
  },
  {
    ...
    hint: "How thick a ring's wall is drawn, the same for every bubble whatever its size — which is what a real bubble looks like, and what a wall measured as a fraction of the radius got wrong: it made a small bubble look like it had a thicker wall than a large one. Does nothing while bubbles are drawn as discs.",
  },
  {
    kind: "slider",
    key: "trail",
...
export const PRESETS: { label: string; hint: string; settings: Settings }[] = [
  {
    label: "time bubbles",
    hint: "Four jets turning the same way under slow, barely-returning water, drawn as hairline rings with a wake behind them. Nothing here holds together for long: the boil shreds every bubble almost as fast as it arrives, and what you are left watching is the flow itself, written in the streaks of several thousand specks. Andrei's scene and his name for it.",
    settings: {
      jets: 4,
      layout: "ring",
      spin: "same",
  ...
  {
    label: "foam",
    hint: "Films that hold, so touching bubbles stay neighbours instead of joining, and the surface packs. `coalesce` is the control that does it — bring it up and the raft dissolves into a few big circles.",
    settings: {
      jets: 4,
      layout: "ring",
      spin: "alternate",
  ...
  {
    label: "standing ring",
    hint: "A closed tub gives back what the jets push out, so somewhere the outflow and the return cancel and the radial flow is zero. Foam arrives there and cannot go further. With churn and waver at zero it comes out as a drawn circle — return moves it in, jet power moves it out, and experiment.stats().ring says where it is.",
    settings: {
      jets: 4,
      layout: "ring",
      spin: "same",
  ...
  {
    label: "ring, breathing",
    hint: "The same standing ring with a little churn and waver under it, so it has lobes and wanders instead of being a perfect circle. One drag apart from the one above, which is the whole point of having both.",
    settings: {
      jets: 4,
      layout: "ring",
      spin: "same",
  ...
  {
    label: "slick",
    hint: "Soapy water: little gas, films that hold a big bubble together, and coalescing so rare that a contact lasts ten seconds. A few large circles drift and burst.",
    settings: {
      jets: 3,
      layout: "scatter",
      spin: "loose",
...
export const TRACKS: Partial<Record<NumericKey, Track>> = {
  ...
  merge: { min: 0.01, max: 60, step: 0.01, scale: "log" },
  rupture: { min: 0, max: 1, step: 0.02 },
...
export const REGISTRY: readonly Slot[] = [
  ...
  { key: "merge", kind: "num", grid: 0.02, origin: 0, bits: 6, retired: true },
  ...
  { key: "rim", kind: "num", grid: 0.01, origin: 0.02, bits: 6, retired: true },
  ...
  // `merge` was "how readily", a 0-to-1 dial scaled by a constant, and it
  // topped out at about 55 joins a second of contact — so a touching pair
  // became one bubble inside a frame and no raft ever existed for `cling` to
  // hold together. Measured at 3,242 merges a second against 1,203 bubbles
  // alive: every bubble merging three times a second. It is a rate now, on a
  // log track, and the bottom of it is where foam lives.
  //
  // `rim` was a fraction of the radius, which made a small bubble look like it
  // had a thicker wall than a large one. It is a width in pixels now, the same
  // for every bubble.
  { key: "merge", kind: "num", grid: 0.01, origin: 0.01, bits: 13 },
  { key: "rupture", kind: "num", grid: 0.02, origin: 0, bits: 6 },
  { key: "rim", kind: "num", grid: 0.1, origin: 0.5, bits: 6 },
