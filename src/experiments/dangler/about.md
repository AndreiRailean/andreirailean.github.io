...
the tree, or the branch, or the strand — only the bulbs, hanging above you at
...
The corollary took a moment to notice and then decided most of the piece: a strand
...
neither is the bend in the strand — without both, there is nothing to see.

A strand fixed at one end with a free end is **not** a catenary; a catenary needs
...
a way you cannot see at three strands and cannot miss at thirty: they stop reading
as one object and start reading as a random spray. Pinned to a surface,
neighbouring strands hang from similar heights, because that is what being
...
Bulbs sit on the sides of the strand rather than on its centreline, alternating as
...
strand reading as a row of identical dots.
...
alone — a short strand under a high ceiling barely fans out at all. **stiffness**
decides whether a strand holds its curl or hangs plumb, and a plumb strand is
...
**sway** and **tremble** are not wind at all — they move the strands by their
...
anchor a few millimetres, and since the strands below do not move with them, all of
them are tugged at the same instant.

Reproducing that as wind failed, and the failure was the useful half. A force
_integrates_: a strand under one keeps accelerating, so a gust of any strength
eventually sweeps a crowd apart. Moving an anchor instead drags its strand by
roughly the anchor's own travel and no further, because the anchor still holds
it. Measured on a short strand, seven millimetres of anchor step settles at
...
The difference is measurable, not only felt. Set forty strands going and ask how
much their tips agree about which way to move: under sway they run at 0.64,
where 1 would be every strand moving as one. Under tremble it is 0.22, and pure
chance for forty strands would be 0.16.
...
Almost none of this was visible on screen. A wrong strand and a right one both
look like a scatter of dots, so nearly every problem below was found by
measuring something rather than by looking at it.

- **A rigid strand hung as limp as a chain.** Bending was measured as the distance
...
- **Then it hung plumb anyway.** Constraining how far a joint bends leaves it
  free to choose a side, and gravity duly picks alternating ones: the strand
...
- **Stiffness as a slider was unusable before it was a physical quantity.** Made
  ...
  enough to hold more crumpled the strand into 90°-per-joint folds, because a
  constraint far from its target pumps energy in on every pass. It scales the
  curvature the strand is asked to hold instead, which keeps the constraint near
  its target and lets the projection stay exact.
- **The coil had a fold in it.** Building the strand's rest shape meant picking two
  directions perpendicular to it, and any way of choosing those flips somewhere
  on the sphere — so as a strand curled past that point, its bending plane jumped.
  It showed only on finely-segmented, strongly-curled strands, which are exactly
  the ones most likely to cross the boundary. The frame is carried along the strand
...
- **A strand that span at two hundred metres a second, for ever.** Moving the
  ...
  by metres, and the strands below did not merely lurch — past about one segment
...
- **The bulbs turned on their strings.** Each bulb hangs off a frame carried
  along its strand, and that frame is built to rotate as little as possible _along
  ...
  accumulate down the strand, so the frame at the free end swung even where
  nothing was bent oddly, and the bulbs riding it went round and round. The
  frame is now carried through time as well as along the strand. Every test of it
  ...
  as a moment ago.
- **The cure for the spinning froze the whole page.** Strands thrown by a
  ...
  because a strand thrown that far never converges and the settle ran to its
  limit each time. The sliders are the instrument here; one that stalls under
  the hand is useless however good the scene behind it is. Strands are now simply
  _carried_ to where their anchors went, which costs about a millisecond and
  disturbs nothing, because a hanging strand's shape never depended on where it
...
- **A screenshot could not tell a settled strand from a falling one.** Stills taken
...
flat arrays; at three strands that is a wash, and at a hundred it is the difference
...
so strand seven is the same strand whether the scene holds eight or eighty. Raising
the count adds strands beside the ones already there instead of rearranging what
you were looking at.
